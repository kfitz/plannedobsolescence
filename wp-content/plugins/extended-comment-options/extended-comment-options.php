...
function eco_extended_comments_options() {
	if (function_exists('add_options_page') )
		add_submenu_page('edit-comments.php', 'Extended Comment Options', 'Batch Status', 'moderate_comments', basename(__FILE__), 'comment_conf');
	...
	*/
	...
	if (DEBUGGING === TRUE) {
		if ( defined('ECO_QUERYVARS') ) {
			define ('ECO_QUERYVARS2', esc_html("[$which] [$status] [$horizon] [$criterion] [$excluded]"));
		} else {
			define ('ECO_QUERYVARS', esc_html("[$which] [$status] [$horizon] [$criterion] [$excluded]"));
		}
		
		if ( defined('ECO_SQL') ) {
			define ('ECO_SQL2', esc_html($sql));
		} else {
			define ('ECO_SQL', esc_html($sql));
		}
	}
...
if ( ( ! empty( $_POST['submit'] ) && $_POST['submit'] == 'Update' ) || ( ! empty( $_POST['for'] ) && $_POST['for'] == 'cancel-auto') ) {
	$eco_clean['submitted'] = true;
}

$eco_html = array();
$eco_html['db_excluded_posts'] = esc_html($eco_clean['db_excluded_posts']);
...
<?php if (true == $auto_check) : ?>
	...
	<?php
    $toappend = " Uncheck the Auto checkbox to disable this option.";
	
	switch ($options['for']) {
		case 'for-all':
			echo "<p>{$options['message_which']} are {$options['status']} on this blog.$toappend</p>";
			break;
		case 'for-bora':
			$date = date('l jS F, Y', strtotime($options['year'].'-'.$options['month'].'-'.$options['day']));
			echo "<p>{$options['message_which']} on posts made before $date are {$options['status']} on this blog.$toappend</p>";
			break;
		case 'for-last':
			if ('posts' == $options['units']) {
				echo "<p>{$options['message_which']} are {$options['status']} on all but the last {$options['last']} posts.$toappend</p>";
			} else {
				$display_units = (1 == $options['last']) ? rtrim($options['units'], 's') : $options['units'];
				echo "<p>{$options['message_which']} are {$options['status']} on posts older than {$options['last']} $display_units.$toappend</p>";
			}
			break;
		case 'for-comments':
			echo "<p>{$options['message_which']} are {$options['status']} on posts with more than {$options['number-comments']} comments.$toappend</p>";
			break;
	}
...
$eco_discussion = empty( $_POST['discussion'] ) ? '' : $_POST['discussion'];
switch($eco_discussion) {
	case 'both':
...
// Which posts?
$eco_for = empty( $_POST['for'] ) ? '' : $_POST['for'];
switch ($eco_for) {
	case 'for-new':
	case 'for-all':
	case 'for-bora':
...
$eco_status = empty( $_POST['status'] ) ? '' : $_POST['status'];
switch ($eco_status) {
	case 'open':
...
if ( ! empty( $_POST['auto'] ) && $_POST['auto'] == 'auto' ) {
	$eco_clean['auto'] = true;
}
...
foreach ($eco_clean as $key => $value) {
	$eco_html[$key] = esc_html($value);
}

$eco_html['status_pp'] = ( ! empty( $eco_clean['status'] ) && $eco_clean['status'] == 'open') ? 'opened' : 'closed';
$eco_html['opp_pp'] = ( ! empty( $eco_clean['opposite_status'] ) && $eco_clean['opposite_status'] == 'open') ? 'opened' : 'closed';
...
if (DEBUGGING === TRUE) {
	function eco_debugging() { ?>
		...
		<pre style="border: 1px solid black; background: #fafafa; padding: 5px;"><?php
			...
			$eco_POST_html = array();
			foreach ($_POST as $key => $value) {
				$eco_POST_html[$key] = esc_html($value);
			}
			echo '<strong>$_POST</strong>:'."\n"; print_r($eco_POST_html); echo "\n";
			
			foreach ($eco_clean as $key => $value) {
				$eco_clean_html[$key] = esc_html($value);
			}
...
$eco_clean['status'] = empty( $eco_clean['status'] ) ? '' : $eco_clean['status'];
$eco_clean['which'] = empty( $eco_clean['which'] ) ? '' : $eco_clean['which'];
...
if ( ! empty( $eco_clean['future'] ) && ! empty($eco_clean['for']) && $eco_clean['future'] === true && $eco_clean['for'] != 'for-new') {
 ...
 */
function eco_call_donation() {

    // Kill notice
    if ( isset( $_GET['remove_eco_donation'] ) )
        update_option( 'eco_show_donation', ECOVERSION );
    
    if ( version_compare( get_option( 'eco_show_donation' ), ECOVERSION, '<' ) )
        add_action( 'admin_notices', 'eco_donation_notice' );
    
}

/**
 * This displays the option to send a donation
 */
function eco_donation_notice() {

    $paypal_link = 'https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=XZCPESHLAFGVU';
    $no_thanks = esc_url( admin_url( 'plugins.php?remove_eco_donation=true' ) );
    echo "<div class='update-nag'>" . sprintf( __( "Thanks for upgrading the 'Extended Comment Options' plugin. Would you consider sending the developer $5.00 USD to sustain development? <a href='%s'>Yes, take me to PayPal</a> | <a href='%s'>No thanks</a> | <a href='%s'>I already did</a>." ), $paypal_link, $no_thanks, $no_thanks ) . "</div>";
}
add_action( 'admin_head', 'eco_call_donation' );
?>
